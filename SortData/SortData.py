# You are given data in a tabular format. The data contains N rows, and each row contains M space separated elements.
#
# You can imagine the M items to be different attributes, (like height, weight, energy, etc.) and each of the N rows as an instance or a sample.
#
# Your task is to sort the table based on the Kth attribute and print the final resulting table. Note that K is indexed from 0 to M-1.
#
# Note: If two attributes are the same for different rows, print the row that appeared first in the input.
#
# Input Format
#
# The first line contains N and M separated by a space.
# The next N lines each contain M elements.
# The last line contains K.
#
# Constraints
#
#  1 <= N, M <= 1000
#  0 <= K < M
# Each element <= 1000
#
# Output Format
#
# Print the N lines of the sorted table. Each line should contain the space separated elements. Check the sample below for clarity.
#
# Sample Input
#
# 5 3
# 10 2 5
# 7 1 0
# 9 9 9
# 1 23 12
# 6 5 9
# 1
# Sample Output
#
# 7 1 0
# 10 2 5
# 6 5 9
# 9 9 9
# 1 23 12


n,m =map(int, input().split())
a = []
for i in range(n):
    a.append(list(map(int, input().split())))
k = int(input())
sorting = sorted(a, key=lambda record: record[k])
for i in sorting:
    print(*i)
